...
protected [sql] final class GeneralDiskHashedRelation(partitions: Array[DiskPartition])
  ...
  override def getIterator() = {
    var ret : Iterator[DiskPartition] = null
    ret = partitions.iterator
    ret
  }

  override def closeAllPartitions() = {
    /* IMPLEMENT THIS METHOD */
    partitions.foreach(DiskPartition => DiskPartition.closePartition())
  }
}
...
private[sql] class DiskPartition (
  ...
  def insert(row: Row) = {
    /* IMPLEMENT THIS METHOD */
    if (inputClosed) {
      throw new SparkException("input was closed")
    }
    data.add(row)
    ...
    new Iterator[Row] {
      ...
      private[this] def fetchNextChunk(): Boolean = {
        ...
        if (!chunkSizeIterator.hasNext) {
          ...
          val chunk_size = chunkSizeIterator.next()
          val byte_data = getNextChunkBytes(inStream, chunk_size, byteArray)
          val list = getListFromBytes(byte_data)
          byteArray = byte_data
          if (list.size() == 0) {
            false
          }else {
...
private[sql] object DiskHashedRelation {
  ...
  def apply (
              ...
              blockSize: Int = 64000) = {
    /* IMPLEMENT THIS METHOD */
    val partition = new Array[DiskPartition](size)
    for (i <- 0 until size) {
      partition(i) = new DiskPartition("partition " + i, blockSize)
    }
    input.foreach(Row => partition(keyGenerator(Row).hashCode() % size).insert(Row))
    partition.foreach(DiskPartition => DiskPartition.closeInput())
    new GeneralDiskHashedRelation(partition)
  }
}
