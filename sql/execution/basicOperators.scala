...
case class PartitionProject(projectList: Seq[Expression], child: SparkPlan) extends UnaryNode {
  ...
  def generateIterator(input: Iterator[Row]): Iterator[Row] = {
    ...
    /* IMPLEMENT THIS METHOD */
      val dist_ite : Iterator[DiskPartition] = DiskHashedRelation(input, keyGenerator).getIterator()
      var curr_block_ite : Iterator[Row] = null

    new Iterator[Row] {
      def hasNext() = {
        /* IMPLEMENT THIS METHOD */
        if (curr_block_ite == null || !curr_block_ite.hasNext) {
          fetchNextPartition()
        }else {
          true
        }
      ...
      private def fetchNextPartition(): Boolean  = {
        /* IMPLEMENT THIS METHOD */
        if (dist_ite.hasNext) {
          val temp = CS143Utils.generateCachingIterator(projectList, child.output)
          curr_block_ite = temp(dist_ite.next().getData())
          true
        }else {
          false
        }
