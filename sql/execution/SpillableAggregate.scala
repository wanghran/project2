...
case class SpillableAggregate(
  ...
  /** Physical aggregator generated from a logical expression.  */
    private val temp = aggregateExpressions.flatMap { agg =>
      agg.collect {
  ...
  private[this] val aggregator: ComputedAggregate = temp(0)
  ...
  def generateIterator(input: Iterator[Row], memorySize: Long = 64 * 1024 * 1024, numPartitions: Int = 64): Iterator[Row] = {
    ...
    def initSpills(): Array[DiskPartition] = {
      ...
      val arr : Array[DiskPartition] = new Array[DiskPartition](numPartitions)
      for (i <- 0 until numPartitions) {
        arr(i) = new DiskPartition("Partition" + i, 0)
      }
      arr
    }
    ...
    new Iterator[Row] {
      ...
      private def aggregate(): Iterator[Row] = {
        ...
        spills.foreach(disk => disk.closeInput())
        val resultProject = AggregateIteratorGenerator(resultExpression, Array(aggregatorSchema) ++ namedGroups.map(_._2))
        resultProject(currentAggregationTable.iterator)
      }
