...
object CS143Utils {
  ...
  def getUdfFromExpressions(expressions: Seq[Expression]): ScalaUdf = {
    ...
    if (expressions.isEmpty) {
      ...
      expressions.foreach(Expression => ret = Expression match {
        case _ScalaUdf : ScalaUdf => _ScalaUdf
        case _ => ret
      })
      ret
    }
    ...
    new Iterator[Row] {
      ...
      def next() = {
        ...
        if (!hasNext()) {
          ...
          val curr = input.next()
          val projection = cacheKeyProjection(curr)
          val seen : Boolean = cache.containsKey(projection)
          if (seen) {
            cache.get(projection)
          }else {
            val ret = preUdfProjection(curr) ++ udfProject(curr) ++ postUdfProjection(curr)
            val row = Row.fromSeq(ret)
            cache.put(projection, row)
            row
          }
        }
      }
  ...
  def apply(resultExpressions: Seq[Expression],
    ...
    new Iterator[Row] {
      ...
      def next() = {
        /* IMPLEMENT THIS METHOD */
        if (hasNext()) {
          val (row, aggreFunc) = input.next()
          val aggreResult = new GenericMutableRow(1)
          val joinRow = new JoinedRow4

          aggreResult(0) = aggreFunc.eval(EmptyRow)
          postAggregateProjection(joinRow(aggreResult, row))
        }else {
          null
        }
      }
